...
local sandbox = setmetatable({
	...
	__index = function(self, index)
		...
		return rawget(self, index) or genv[index] or fenv[index]
	end
})

--newcclosure by emper and myworld
...
local function randomgen(length)
...
local loadstring = newcclosure(function(source)
	...
	local ran, failureReason = pcall(function()
		...
		executable = createExecutable(compiledBytecode, sandbox)
	end)
	
	if ran then
		return setfenv(executable, sandbox)
	end
...
for _, c in next, game:GetService("CoreGui"):GetDescendants() do
	if c:IsA("ModuleScript") then
		local clone = c:Clone()
		clone.Name = '[string "'..randomgen(11)..'"]'
...
local runningscripts_blacklisted = {game:GetService("CoreGui"), cpkgs, game:GetService("StarterPlayer")}
...
local function execute_cmd(command)
    ...
    linkingsvc:OpenUrl(
        scriptctxt:SaveScriptProfilingData(command, filename)
    )
end
...
if script.Name == "PolicyService" then local PlayersService = players local isSubjectToChinaPolicies = true local policyTable local initialized = false local initAsyncCalledOnce = false local initializedEvent = Instance.new("BindableEvent") local PolicyService = {} function PolicyService:InitAsync() if _G.__TESTEZ_RUNNING_TEST__ then isSubjectToChinaPolicies = false return end if initialized then return end if initAsyncCalledOnce then initializedEvent.Event:Wait() return end initAsyncCalledOnce = true local localPlayer = PlayersService.LocalPlayer while not localPlayer do PlayersService.PlayerAdded:Wait() localPlayer = PlayersService.LocalPlayer end assert(localPlayer, "") pcall(function() policyTable = game:GetService("PolicyService"):GetPolicyInfoForPlayerAsync(localPlayer) end) if policyTable then isSubjectToChinaPolicies = policyTable["IsSubjectToChinaPolicies"] end initialized = true initializedEvent:Fire() end function PolicyService:IsSubjectToChinaPolicies() self:InitAsync() return isSubjectToChinaPolicies end ret_val = PolicyService elseif script.Name == "JestGlobals" then local escape = Enum.KeyCode.Escape local input_manager = Instance.new("VirtualInputManager") input_manager:SendKeyEvent(true, escape, false, game) input_manager:SendKeyEvent(false, escape, false, game) input_manager:Destroy() end
...
repeat twait() 
until script.Name == "Nyx"
script.Name = "JestGlobals" return require(script) end


function shitassrequire(urd)
return require(urd)
end
